...
export async function main() {
  ...
  const rows = await supabaseRequest(`/rest/v1/app_cpe_portal_sync_jobs?select=id,chapa,portal_password,security_key,status,expires_at&id=eq.${encodeURIComponent(jobId)}&limit=1`);
  const job = rows?.[0];
