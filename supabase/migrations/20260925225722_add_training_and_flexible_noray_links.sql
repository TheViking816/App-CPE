-- Allow only the three verified iframe paths; each destination still uses
-- the authenticated worker's own chapa, registration and stored credential.
...
begin
  ...
  v_path := case p_section
    ...
    when 'excluir-jornadas' then 'excluir-jornadas'
    ...
    when 'puntos-formacion' then 'puntos-formacion'
    when 'situacion-trabajador' then 'situacion-trabajador'
    when 'solicito-formacion' then 'solicito-formacion'
    else null
  end;
