...
alter table public.app_cpe_worker_commands
  ...
  check (command_type in ('pending_sync', 'current_month_all'));

create or replace function public.app_cpe_admin_request_current_month_worker(p_token text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  where command_type = 'current_month_all'
  ...
  if v_command.id is null then
    ...
    values ('current_month_all', v_admin.chapa)
...
begin
  ...
  from public.app_cpe_worker_commands
  where status = 'queued'
    and command_type in ('pending_sync', 'current_month_all')
...
revoke all on function public.app_cpe_admin_request_current_month_worker(text) from public, anon, authenticated;
grant execute on function public.app_cpe_admin_request_current_month_worker(text) to anon, authenticated;
