...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    ...
    navigator.serviceWorker.addEventListener("controllerchange", () => {
      forceReload("20260815-hide-partial-warning-1");
    });
    navigator.serviceWorker.addEventListener("message", (event) => {
      if (event.data?.type === "APP_CPE_FORCE_RELOAD") forceReload(event.data.version);
    });

    navigator.serviceWorker.register(`${import.meta.env.BASE_URL}service-worker.js?v=20260815-hide-partial-warning-1`, {
      updateViaCache: "none"
    }).then((registration) => {
