...
export default function GeneralBoard({ chapa, onOpen }) {
  ...
  return (
    <section className="general-board page-panel">
      ...
      {!loading && journey && <>
        <div className="general-journeys">{data.journeys.map((item) => { const itemCounts = boardCounts(item); return <button type="button" className={item.key === journey.key ? "active" : ""} onClick={() => { setSelected(item.key); setExpandAll(false); }} key={item.key}><strong>{item.jornada}</strong><span>{dateLabel(item.fecha)}</span><small>{itemCounts.total} asignaciones</small><em><b>B {itemCounts.bolsa}</b><b>T {itemCounts.turno}</b></em></button>; })}</div>
